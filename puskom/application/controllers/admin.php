...
class Admin extends CI_Controller {
	...
	public function index(){	
		...
		$this->load->view('admin/v_navigation');
	...
	public function user(){	
		...
		$where = "stf_id not in (SELECT stf_id from staff where stf_id = ".$id.") and staff.sst_id = staffstatus.sst_id";

		$data ['Staff'] = $this->masterData->selectJoin($select, $table, $where);
	...
	public function tambahUser(){
		$data['status'] = $this->masterData->getData('staffstatus');
		$this->load->view('header');
		$this->load->view('admin/v_navigation');
	...
	public function editUser($id){
		$where = ('stf_id ='.$id);
		$data['user'] = $this->masterData->getWhereData('staff',$where);
		$data['status'] = $this->masterData->getData('staffstatus');
		$this->load->view('header');
		$this->load->view('admin/v_navigation');
	...
	public function simpanUser(){
		//if(isset($_post['Simpan'])){
			...
			$this->masterData->inputData($data,'staff');
			redirect('admin/user');
	}
	...
	public function deleteUser($id){
		...
		$this->masterData->deleteData($where,'staff');
		redirect('admin/user');
	}
	...
	public function saveProfile(){
		...
		if($cekStaff > 0){
			...
			$this->masterData->editData($where,$data,'staff');
			$success = 'true';
			redirect(base_url("admin/showProfile/".$success));
		} else {
			$success = 'false';
			redirect(base_url("admin/showProfile/".$success));
		}
	...
	public function showMahasiswa(){
		...
		$this->load->view('header');
		$this->load->view('admin/v_navigation');
		$this->load->view('admin/v_existedMhs',$data);
		$this->load->view('footer');
	}
	...
	public function tampil_jadwal(){
    	...
    	$this->load->view('admin/cetak_jadwal', $data);
    }
    ...
    public function cetak_sertifikat($pstr_id){
    	...
    	$data['peserta'] = $this->masterData->selectJoin($select,$table,$where);
    	$this->load->view('admin/v_cetak_sertifikat',$data);
    }
    ...
    public function cetak_sertifikat_all($jd_id){
		...
		$data['peserta'] = $this->masterData->selectJoin($select,$table,$where);
		$this->load->view('admin/v_cetak_sertifikat_all',$data);
	}
	...
	public function tambahJadwal(){
		...
		$this->load->view('header');
		$this->load->view('admin/v_navigation');
		$this->load->view('admin/v_tambah_jadwal', $data);
		$this->load->view('footer');	
	}	
	...
	public function insertJadwal(){
		...
		redirect('admin/viewJadwal', $data);
	}
	...
	public function selectPeserta($jd_id){
		...
		$this->load->view('header');
		$this->load->view('admin/v_navigation');
		$this->load->view('admin/v_selectedPeserta', $data);
		$this->load->view('footer');
	}
	...
	public function cetak_peserta($jd_id){
		...
		$data ['Peserta'] = $this->masterData->selectJoin($select, $table, $where);	
    	$this->load->view('admin/cetak_peserta', $data);
    }
	...
	public function inputPeserta(){
		...
		$this->load->view('header');
		$this->load->view('admin/v_navigation');
		$this->load->view('admin/v_inputPeserta',$data);
		$this->load->view('footer');
	}
	...
	public function insertPeserta(){
		...
		redirect(base_url()."admin/selectPeserta/".$jd_id);
	}
	...
	public function inputNilai(){
		...
		$this->load->view('header');
		$this->load->view('admin/v_navigation');
		$this->load->view('admin/v_inputnilai',$data);
		$this->load->view('footer');
	}
...
// ============================================= Sync ==================================================== //
	...
	public function sinkronFak(){
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => "http://192.168.1.11/api_puskom/fakultas",
	...
	public function sinkronProdi(){
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => "http://192.168.1.11/api_puskom/prodi",
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => "",
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 30,
		...
		if ($err) {
		  //echo "cURL Error #:" . $err;
		  $this->session->set_flashdata('hasil','Sinkron Failed, cURL Error #:'. $err);
		  redirect('admin/viewProdi');
		} else {
			if ($response == 'Illegal Access') {
				$this->session->set_flashdata('hasil','Sinkron Failed, <b>Illegal Access!!</b>');
				redirect('adminviewProdi');
			}else{
	...
	public function sinkronMhs(){
		...
		// 	'pst_id'
		// );
		//$mhs = $this->masterData->getDataGroup($pilih,'mahasiswa',$grup);
		// $ono = 0;
		// foreach ($mhs->result() as $key) {
		// 	if ($key->mhs_tahunmasuk==$thn AND $key->pst_id==$prodi) {
		// 		$ono = 1;
		// 	}
		// }
		// if($ono==1){
		// 	echo "<script>
		// 				alert(\"Data Sudah Ada Dabss!!\");
		// 				history.back(-1);
		// 			</script>";
		// }
		// 	else{
